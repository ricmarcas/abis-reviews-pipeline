...
async function sortByMostRecent(page) {
  ...
  async function clickMostRecentOption() {
    ...
    if (!target) return { ok: false, targetIndex: -1, targetText: "" };

    const options = page.locator('div[role="menu"] [role="menuitemradio"]');
    const targetOption = options.nth(target.index);
    await targetOption.click({ timeout: 5000, force: true });
    return { ok: true, targetIndex: target.index, targetText: target.text };
  }
  ...
  if (!reopened) return;
  await sleep(400);
  let selected = await isMostRecentSelected();
  let menuRows = await getMenuOptions();

  if (!selected && clicked.targetIndex >= 0) {
    // Fallback: keyboard selection from top of menu.
    await page.keyboard.press("Home").catch(() => {});
    for (let i = 0; i < clicked.targetIndex; i += 1) {
      await page.keyboard.press("ArrowDown").catch(() => {});
    }
    await page.keyboard.press("Enter").catch(() => {});
    await sleep(1000);

    const reopenedAfterKeyboard = await openSortMenu();
    if (reopenedAfterKeyboard) {
      await sleep(300);
      selected = await isMostRecentSelected();
      menuRows = await getMenuOptions();
    }
  }

  console.info("ABIS sort menu status", {
    selected_most_recent_menu_signal: selected,
    target: clicked.targetText,
    options: menuRows,
    note: "En algunos layouts de Google Maps, aria-checked no refleja el estado real del orden.",
  });
  await page.keyboard.press("Escape").catch(() => {});
}
...
export async function scrapeGoogleMapsReviews(locationUrl, options = {}) {
  ...
  try {
    ...
    const extractedAgeDays = allReviews
      .map((review) => parseRelativeDate(review.date_text))
      .filter((days) => Number.isFinite(days));
    const appearsSortedByRecency = extractedAgeDays.every((days, index) => {
      if (index === 0) return true;
      return extractedAgeDays[index - 1] <= days;
    });
    console.info("ABIS extracted review date order", {
      appears_sorted_by_recency: appearsSortedByRecency,
      sample_date_texts: allReviews.slice(0, 6).map((item) => item.date_text),
      effective_sort_confirmation:
        appearsSortedByRecency
          ? "confirmed_by_dates"
          : "not_confirmed_by_dates",
    });
